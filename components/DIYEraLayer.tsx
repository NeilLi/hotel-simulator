...
/**
 * DIY Era — Creative Studio Hub
 * A lightweight router between the hub and the selected creative module.
 */

const cx = (...parts: Array<string | false | undefined | null>) =>
  parts.filter(Boolean).join(" ");

type ThemeKey = "blue" | "gold" | "rose" | "royal";
type ZoneId = "JOURNEY" | "GIFT" | "WEAR" | "KIDS";
...
const STYLES = `
@keyframes blobPulse { 0%{transform:scale(1);opacity:.5} 50%{transform:scale(1.1);opacity:.7} 100%{transform:scale(1);opacity:.5} }
@keyframes bounce { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-5px)} }
@keyframes spin-slow { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
@keyframes gradient-x { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }
@keyframes firework-burst { 0%{transform:scale(0);opacity:0} 15%{opacity:1;transform:scale(.5)} 50%{opacity:.8} 100%{transform:scale(1.5);opacity:0} }
@keyframes firework-particles { 0%{transform:translate(-50%,-50%) scale(1);opacity:1} 100%{transform:translate(calc(-50% + var(--tx)),calc(-50% + var(--ty))) scale(0);opacity:0} }
...
> = {
  blue: {
    bg: "bg-blue-50",
    borderHover: "border-blue-300 hover:border-blue-500",
    text: "text-blue-900",
  ...
  gold: {
    bg: "bg-amber-50",
    borderHover: "border-amber-200 hover:border-amber-400",
    text: "text-amber-900",
  ...
  royal: {
    bg: "bg-indigo-50",
    borderHover: "border-indigo-200 hover:border-indigo-400",
  ...
  rose: {
    bg: "bg-rose-50",
    borderHover: "border-rose-200 hover:border-rose-400",
    text: "text-rose-900",
...
const ZONES: Zone[] = [
  ...
  { id: "KIDS", name: "Magic Atelier", caption: "Build Companions", icon: Sparkles, theme: "rose", isKids: true },
];

const MODULES: Record<ZoneId, React.ComponentType<{ onBack: () => void }>> = {
  WEAR: WearableStoryStudio,
  KIDS: MagicAtelier,
  JOURNEY: JourneyStudio,
  GIFT: GiftForgePage,
};

// --- FIREWORKS ---
type FireworkProps = { x: number; y: number; color: string; delay: number; scale?: number };

function Firework({ x, y, color, delay, scale = 1 }: FireworkProps) {
  const particles = useMemo(() => {
    const count = 14;
    return Array.from({ length: count }, (_, i) => {
      const angle = (i / count) * Math.PI * 2;
      const velocity = 70 + Math.random() * 70;
      return { tx: Math.cos(angle) * velocity, ty: Math.sin(angle) * velocity };
    });
  }, []);
  ...
  return (
    <div
      ...
      style={{ left: `${x}%`, top: `${y}%`, transform: `translate(-50%, -50%) scale(${scale})`, willChange: "transform" }}
    >
      <div className="relative" style={{ animation: `firework-burst 2.8s ease-out infinite`, animationDelay: `${delay}ms` }}>
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 rounded-full blur-3xl opacity-40" style={{ backgroundColor: color }} />
        <div
...
const FIREWORKS: Array<FireworkProps> = [
...
function FireworksBackground() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {FIREWORKS.map((fw, i) => (
        // @ts-expect-error - key is a special React prop, not part of FireworkProps
        <Firework key={i} {...fw} />
      ))}
    </div>
  );
  ...
  return (
    <div className="relative z-20 flex-shrink-0 flex items-center justify-between px-6 py-6 md:px-10">
      <div className="flex items-center gap-4">
        <button
          ...
          className="group inline-flex items-center gap-2 rounded-full border border-blue-300 bg-white/80 backdrop-blur-md px-5 py-2.5
                     text-[11px] font-bold uppercase tracking-widest text-blue-700 hover:text-blue-800 hover:border-blue-500
                     hover:shadow-lg hover:shadow-blue-200 transition-all focus:outline-none focus:ring-4 focus:ring-blue-200"
        ...
        <div className="hidden sm:flex items-center gap-3">
          <div className="flex items-center gap-2 text-blue-900">
            ...
            <span className="text-[12px] font-bold tracking-widest uppercase text-blue-800">SeedCore</span>
          </div>
      ...
      <div className="hidden md:flex items-center gap-3">
        <div className="flex -space-x-2" aria-hidden="true">
          {["A", "B", "C"].map((ch, idx) => (
            <div
              key={ch}
              className={cx(
                "w-7 h-7 rounded-full border-2 border-white flex items-center justify-center text-[9px] font-bold text-white shadow-sm",
                idx === 0 ? "bg-blue-400" : idx === 1 ? "bg-amber-400" : "bg-rose-400"
              )}
            >
              {ch}
            </div>
        ...
        <span className="text-[10px] font-bold tracking-widest uppercase text-slate-400">3 creators active</span>
      </div>
    </div>
  );
}

function PortalCTA({ onExplore }: { onExplore: () => void }) {
  return (
    <div className="relative z-10 w-full mb-10">
      <div className="relative overflow-hidden rounded-[2rem] border border-blue-300 bg-white shadow-[0_20px_50px_-10px_rgba(37,99,235,0.25)]
                      hover:shadow-[0_30px_60px_-10px_rgba(37,99,235,0.3)] transition-all duration-500">
        ...
        <div className="relative p-10 md:p-14 flex flex-col md:flex-row items-center md:items-start justify-between gap-10">
          <div className="flex-1 text-center md:text-left z-10">
            ...
            <p className="max-w-xl text-slate-600 text-base font-medium leading-relaxed mb-10 mx-auto md:mx-0">
              Pick a studio below. Your AI tools will help you explore ideas, iterate fast, and ship something real.
            </p>
          ...
          <div className="relative hidden md:block w-72 h-72 flex-shrink-0" aria-hidden="true">
  ...
  return (
    <button
      ...
      className={cx(
        ...
        "transition-all duration-300 outline-none flex flex-col justify-between border-[3px]",
        "focus:ring-4 focus:ring-blue-200",
        focused
          ? `bg-white scale-[1.02] shadow-2xl ${theme.shadow} z-10 border-transparent`
          : `bg-white border-slate-100 ${theme.borderHover} shadow-sm hover:shadow-lg`,
        zone.isKids && !focused && "bg-gradient-to-br from-rose-50 to-purple-50 border-rose-100"
      )}
      aria-label={`Open ${zone.name}`}
    >
      <div className={cx("absolute top-0 right-0 w-48 h-48 rounded-bl-full transition-all duration-500 ease-out", theme.bg, focused ? "opacity-100 scale-100" : "opacity-0 scale-75")} />
      ...
      <div className="relative p-7 flex flex-col h-full justify-between z-10">
        <div className="flex flex-col gap-5">
          <div className={cx("w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-md", focused && "scale-110 rotate-3", theme.iconBg)}>
            <Icon size={26} />
          </div>

          <div>
            <div className={cx("text-[17px] font-bold tracking-tight leading-tight", focused ? "text-slate-900" : "text-slate-700")}>
              {zone.name}
            </div>
            <div className="mt-1.5 flex items-center gap-2">
              <div className={cx("w-8 h-0.5 rounded-full transition-colors", focused ? theme.accent : "bg-slate-200")} />
              <div className="text-[10px] font-bold tracking-widest uppercase text-slate-400 group-hover:text-slate-500">{zone.caption}</div>
            </div>
          </div>
        </div>

        <div className="flex items-center justify-between mt-4">
          <div className="flex -space-x-2 opacity-0 group-hover:opacity-100 transition-opacity delay-75" aria-hidden="true">
            {[0, 1, 2].map((i) => (
              <div key={i} className="w-6 h-6 rounded-full bg-slate-100 border-2 border-white shadow-sm" />
            ))}
          </div>

          <div className={cx("w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300", focused ? `text-white shadow-lg translate-x-1 ${theme.accent}` : "bg-slate-50 text-slate-300")}>
...
export function DIYEraLuxPortal({
  ...
  logoSrc?: string; // kept for API compatibility (unused here). Remove if not needed across app.
}) {
  const [ready, setReady] = useState(false);
  const [focus, setFocus] = useState<ZoneId>("JOURNEY");
  const [activeModule, setActiveModule] = useState<ZoneId | null>(null);
  ...
  const focusedZone = useMemo(() => ZONES.find((z) => z.id === focus) ?? ZONES[0], [focus]);
  ...
  if (activeModule) {
    ...
    return <Module onBack={() => setActiveModule(null)} />;
  }

  // --- HUB ---
  return (
    <div ref={wrapRef} onMouseMove={onMouseMove} className="relative h-screen w-full bg-[#FAFAFA] text-slate-900 flex flex-col overflow-hidden">
      <style>{STYLES}</style>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide relative z-10 w-full min-h-0">
        <div
          className={cx(
            "max-w-7xl mx-auto px-6 pb-20 pt-2 md:px-10 w-full transition-all duration-700",
            ready ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
          )}
        >
          <PortalCTA onExplore={() => openZone(focus)} />

          <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" style={{ gridAutoRows: "1fr" }}>
            {ZONES.map((z) => (
              <div key={z.id} className="flex h-full">
                <DoorTile zone={z} focused={focus === z.id} onFocus={() => setFocus(z.id)} onOpen={() => openZone(z.id)} />
              </div>
            ))}
          </div>
          ...
          <div className="w-full">
            <div className={cx("relative rounded-[2rem] overflow-hidden bg-white shadow-2xl transition-all duration-500 border-4 w-full", "border-slate-50 shadow-slate-200")}>
              <div className={cx("h-40 w-full relative overflow-hidden", activeTheme.bg)}>
                <div className="absolute inset-0 bg-gradient-to-t from-white via-transparent to-transparent opacity-80" />
                <div className="absolute top-4 right-4 opacity-20">
                  <Sparkles size={48} className={activeTheme.text} />
                </div>
                <div className="absolute bottom-4 left-4 w-24 h-24 rounded-full bg-slate-900/10 blur-xl" />
              </div>
              ...
              <div className="relative px-8 pb-10 -mt-16 flex flex-col">
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <h2 className="text-4xl font-black text-slate-900 mb-4 leading-tight">{focusedZone.name}</h2>
                  <p className="text-[15px] font-medium text-slate-500 leading-relaxed mb-10">
                    {focusedZone.id === "KIDS"
                      ? "Create playful companions with personalities, skills, and behaviors. Great for kid-friendly experiences and imaginative storytelling."
                      : `Jump into ${focusedZone.name}. Your AI helpers will guide concepts, variations, and a clean final output.`}
                  </p>
                </div>

                <div className="mt-auto pt-4">
                  <button
                    onClick={() => openZone(focusedZone.id)}
                    className={cx(
                      "w-full group relative overflow-hidden inline-flex items-center justify-center gap-3 rounded-2xl py-4 text-xs font-bold uppercase tracking-widest transition-all active:scale-95 shadow-lg text-white focus:outline-none focus:ring-4 focus:ring-blue-200",
                      activeTheme.iconBg
                    )}
                  >
                  ...
                  <div className="mt-5 flex justify-center text-[10px] font-bold text-slate-400 uppercase tracking-widest items-center gap-2">
